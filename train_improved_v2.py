...
def evaluate_agent(agent, env, num_episodes=20):
    ...
    for _ in range(num_episodes):
        ...
        while not done:
            ...
            action, _, _ = agent.select_action(state, valid_actions=valid_actions, epsilon=0.0)
...
class AdvancedPPOTrainer:
    def __init__(self, config, device='cpu'):
        ...
        self.agent.optimizer = optim.Adam(self.agent.network.parameters(), lr=ppo_config['lr'], eps=1e-8)
        
        self.best_score = 0
        self.no_improve_count = 0
        self.patience = 15  # 提高耐心值
    ...
    def train_episode(self, epsilon=0.0):
        ...
        while not done:
            ...
            action, log_prob, value = self.agent.select_action(
                state, valid_actions=valid_actions, epsilon=epsilon
            )
    ...
    def train(self, num_episodes=5000):
        ...
        print("🚀 开始改进版训练 V2 - 集成所有优化")
        print(f"    One-hot编码: 开启")
        print(f"    网络大小: 256隐藏层 + LayerNorm + Dropout")
        print(f"    学习率: {self.initial_lr} -> {self.final_lr}")
        print(f"    探索率: {initial_epsilon} -> {final_epsilon}")
        print(f"    早停耐心: {self.patience} 次评估")
        ...
        for episode in tqdm(range(num_episodes), desc="Training"):
            ...
            current_lr = linear_lr_schedule(
                self.agent.optimizer, self.initial_lr, self.final_lr, num_episodes, episode
            )
            ...
            if (episode + 1) % self.config.eval_interval == 0:
                eval_score, avg_max_tile, best_max_tile = evaluate_agent(
                    self.agent, self.env, self.config.eval_episodes
                )
                print(f"🎯 Evaluation - "
                ...
                if eval_score > self.best_score:
                    ...
                    self.no_improve_count = 0
                    save_path = os.path.join(self.config.model_save_path, 'ppo_best_improved_v2.pth')
            ...
            if (episode + 1) % self.config.save_interval == 0:
                save_path = os.path.join(self.config.model_save_path, f'ppo_episode_{episode + 1}_v2.pth')
                os.makedirs(os.path.dirname(save_path), exist_ok=True)
                self.agent.save(save_path)
    ...
    def plot_metrics(self, intermediate=False):
        ...
        if self.metrics['learning_rates']:
            ...
            ax.set_title('Learning Rate and Exploration Schedule')
            ax.grid(True, alpha=0.3)
        ...
        if self.metrics['best_max_tiles']:
            ...
            ax.set_ylabel('Count')
            ax.set_title('Max Tile Distribution (during evaluations)')
            ax.grid(True, alpha=0.3)
        ...
        if intermediate:
            save_path = os.path.join('logs', f'training_progress_v2_ep{self.metrics["episodes"][-1]}.png')
        else:
            save_path = os.path.join('logs', 'training_progress_v2_final.png')
...
def main():
    parser = argparse.ArgumentParser(description='Improved RL 2048 Training V2')
    ...
    config.eval_interval = 200  # 更频繁评估
